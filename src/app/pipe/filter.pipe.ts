...
export class FilterPipe implements PipeTransform {
  ...
  transform(value: any[] | null, filmTitle: string, filmYearFrom: number, filmYearTo: number, stars: number, filmGenres: string[], filmExcludedGenres: string[], filmKeywords: string[]): Film[] | null {
    ...
    if (value) {
      const filtered = value.filter(item => {
        ...
        return filmTitleMatch && filmYearFromMatch && filmYearToMatch && starsMatch && filmGenresMatch && filmExcludedMatch && filmKeywordsMatch;
      });
